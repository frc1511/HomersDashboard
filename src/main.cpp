...
int main(int argc, char** argv) {
  ...
  while (app.is_running()) {
    if (GraphicsManager::get().poll_events()) {
      app.close();
    }
    ...
    // New Frame.
    GraphicsManager::get().begin_frame();
    ...
    // clang-format off
    if (!ImGui::Begin("Homer's Dashboard", &running,
    ...
    // Render frame.
    GraphicsManager::get().end_frame();
  }

  GraphicsManager::get().deinit();
...
static FontLibrary load_fonts() {
  ...
  font_lib.big_font = io->Fonts->AddFontFromMemoryTTF(
      reinterpret_cast<void*>(Ubuntu_Bold_ttf), Ubuntu_Bold_ttf_size, 50.0f,
      &font_cfg, glyph_ranges);
...
static void setup_data_handler(App& app) {
  ...
  ini_handler.TypeName = app.type_name();
